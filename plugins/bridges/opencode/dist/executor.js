...
import { recordSuccess, shouldSkipHook } from './cache';
 ...
 */
export function executeHook(hook, filePaths, options) {
    const startTime = Date.now();
    // Check cache: skip if every file is unchanged since last successful run
    if (filePaths.length > 0) {
        const allCached = filePaths.every((fp) => shouldSkipHook(hook.pluginName, hook.name, fp));
        if (allCached) {
    ...
    // biome-ignore lint/suspicious/noTemplateCurlyInString: intentional literal match for shell variable
    ...
    // Log hook_run event
    const hookRunId = options.eventLogger?.logHookRun(hook, options.hookType ?? 'PostToolUse');
 ...
 */
export async function executeHooksParallel(hooks, filePaths, options) {
    if (hooks.length === 0)
        return [];
    const results = await Promise.allSettled(hooks.map((hook) => executeHook(hook, filePaths, options)));
