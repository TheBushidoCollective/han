...
```bash
npx @thebushidocollective/han plugin install buki-lerna
```
...
For modern Lerna (v7+) using npm/yarn/pnpm workspaces, the bootstrap command links local packages and installs remaining dependencies. Consider using `buki-npm` or `buki-yarn` alongside this plugin for complete dependency management.
