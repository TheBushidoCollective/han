...
export class SessionEncryptionService {
  private encryptionService = new AsyncEncryptionService();
  ...
  async encryptSession(
    ...
    // Step 2: Scan for secrets with redaction
    const scanResult = this.secretDetector.scanWithRedaction(contentString);
