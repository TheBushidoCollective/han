...
async function validateToken(token: string): Promise<AuthenticatedContext> {
  // SECURITY: Only allow test tokens when explicitly enabled via environment
  // This prevents privilege escalation in production
  if (token.startsWith("test:")) {
    if (process.env.HAN_TEST_MODE !== "true" && process.env.NODE_ENV !== "test") {
