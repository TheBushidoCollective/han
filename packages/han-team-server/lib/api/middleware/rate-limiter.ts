 ...
 *
 * TODO(security): HIGH-5 - This in-memory rate limiter has potential race conditions
 * between the token check and decrement operations. Under high concurrency,
 * more requests than the limit may be allowed. This is accepted MVP debt.
 * For production-critical endpoints, consider:
...
/**
 * Get client IP address safely, respecting TRUST_PROXY configuration
 *
 * SECURITY: Only trusts X-Forwarded-For and X-Real-IP headers when
 * TRUST_PROXY is enabled. This prevents IP spoofing attacks where
 * attackers send fake headers to bypass rate limiting.
 *
...
export function getClientIp(c: Context): string {
  ...
  // Only trust proxy headers if TRUST_PROXY is explicitly enabled
  ...
  // In non-proxy mode or if no proxy headers, use the direct connection IP
  // Note: Hono's raw() returns the underlying request which may have socket info
  // For now, return "unknown" if we can't determine it - this is still safer
  // than blindly trusting potentially spoofed headers
  return "unknown";
}
 ...
 *
 * SECURITY: Uses getClientIp() which respects TRUST_PROXY configuration
 * to prevent IP spoofing attacks on authentication endpoints.
 */
export const authRateLimit = rateLimit({
  ...
  keyGenerator: (c) => {
    ...
    // getClientIp respects TRUST_PROXY to prevent spoofing
    const ip = getClientIp(c);
    return `ip:${ip}`;
  },
});
