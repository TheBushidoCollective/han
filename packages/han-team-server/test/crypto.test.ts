...
describe("Encryption Service", () => {
  ...
  describe("encryptWithKey/decryptWithKey", () => {
    // Note: This test uses a single key derivation and then re-uses the KEK
    // to avoid multiple Argon2id derivations (each takes ~1s with OWASP params)
    it("should encrypt multiple items with same key", () => {
      ...
      // Encrypt multiple items with the same wrapped key (reusing kek)
      const item1 = new TextEncoder().encode("First item");
      const item2 = new TextEncoder().encode("Second item");

      // Unwrap DEK once and use for both encryptions
      const dekUnwrapped = service.unwrapKey(wrappedKey, keyNonce, kek);

      const nonce1 = generateNonce();
      const nonce2 = generateNonce();

      const ciphertext1 = encrypt(item1, dekUnwrapped, nonce1);
      const ciphertext2 = encrypt(item2, dekUnwrapped, nonce2);

      // Decrypt and verify using the same unwrapped DEK
      const decrypted1 = decrypt(ciphertext1, dekUnwrapped, nonce1);
      const decrypted2 = decrypt(ciphertext2, dekUnwrapped, nonce2);
