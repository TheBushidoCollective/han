...
switch (platform) {
    ...
    switch (arch) {
      case 'arm64':
        localFileExisted = existsSync(
          join(__dirname, 'han-native.android-arm64.node')
        );
        try {
          if (localFileExisted) {
      ...
      case 'arm':
        localFileExisted = existsSync(
          join(__dirname, 'han-native.android-arm-eabi.node')
        );
        try {
          if (localFileExisted) {
  ...
  case 'darwin':
    localFileExisted = existsSync(
      join(__dirname, 'han-native.darwin-universal.node')
    );
    try {
      if (localFileExisted) {
    ...
    switch (arch) {
      case 'x64':
        localFileExisted = existsSync(
          join(__dirname, 'han-native.darwin-x64.node')
        );
        try {
          if (localFileExisted) {
  ...
  case 'freebsd':
    ...
    localFileExisted = existsSync(
      join(__dirname, 'han-native.freebsd-x64.node')
    );
    try {
      if (localFileExisted) {
...
const {
  ...
  updateOrchestration,
  cancelOrchestration,
  getOrchestrationHooks,
  queueHook,
  getQueuedHooks,
  deleteQueuedHooks,
  queuePendingHook,
  getPendingHooks,
  getSessionPendingHooks,
  updateHookStatus,
  completeHookExecution,
  failHookExecution,
  getOrCreateHookAttempt,
  incrementHookFailures,
  resetHookFailures,
  increaseHookMaxAttempts,
  recordFrustration,
  ...
  getFilesForValidation,
  deleteStaleValidations,
  ...
  upsertSessionTodos,
  getSessionTodos,
  upsertGeneratedSummary,
  getGeneratedSummary,
  searchGeneratedSummaries,
  listSessionsWithoutSummaries,
  getSessionNativeTasks,
  getNativeTask,
  ...
  getStaleLockTimeout,
  cleanupStaleCoordinatorLock,
  indexSessionFile,
  indexProjectDirectory,
  handleFileEvent,
  fullScanAndIndex,
  enqueueAsyncHook,
  listPendingAsyncHooks,
  isAsyncHookQueueEmpty,
  drainAsyncHookQueue,
  cancelPendingAsyncHooks,
  completeAsyncHook,
  cancelAsyncHook,
