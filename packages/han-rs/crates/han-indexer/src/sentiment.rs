...
pub struct SentimentResult {
    /// Raw VADER compound score scaled to -5..5.
...
/// Analyze sentiment of a message, returning None for empty messages.
pub fn analyze_sentiment(message: &str) -> Option<SentimentResult> {
    ...
    // VADER compound score is -1 to 1; convert to -5 to 5 scale
    ...
    // ALL CAPS detection — strip out environment variable patterns first
    ...
    // Find longest run of consecutive ALL CAPS words
    ...
    // Score based on consecutive caps words — only if sentiment is not positive
    ...
    // Repeated words (adjacent duplicate words)
