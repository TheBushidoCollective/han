	...
	describe("NotebookEdit tool", () => {
		it("extracts notebook_path from NotebookEdit tool input", () => {
			const payload = {
				...
				tool_input: { notebook_path: "/path/to/notebook.ipynb", new_source: "# cell" },
			};
...
describe("matchesFileFilter", () => {
	...
	describe("with no filter", () => {
		it("matches any file when filter is undefined", () => {
			expect(matchesFileFilter("/project/root/src/file.ts", undefined, cwd)).toBe(true);
		});

		it("matches any file when filter is empty array", () => {
			expect(matchesFileFilter("/project/root/src/file.ts", [], cwd)).toBe(true);
		});
	});
	...
	describe("TypeScript/JavaScript patterns", () => {
		...
		it("matches .ts files", () => {
			expect(matchesFileFilter("/project/root/src/index.ts", tsFilter, cwd)).toBe(true);
		});

		it("matches .tsx files", () => {
			expect(matchesFileFilter("/project/root/src/component.tsx", tsFilter, cwd)).toBe(true);
		});

		it("matches deeply nested files", () => {
			expect(matchesFileFilter("/project/root/src/a/b/c/file.ts", tsFilter, cwd)).toBe(true);
		});

		it("does not match .js files", () => {
			expect(matchesFileFilter("/project/root/src/file.js", tsFilter, cwd)).toBe(false);
		});

		it("does not match .json files", () => {
			expect(matchesFileFilter("/project/root/package.json", tsFilter, cwd)).toBe(false);
		});
	});
	...
	describe("Test file patterns", () => {
		...
		it("matches .test.ts files", () => {
			expect(matchesFileFilter("/project/root/src/utils.test.ts", testFilter, cwd)).toBe(true);
		});

		it("matches .spec.ts files", () => {
			expect(matchesFileFilter("/project/root/src/utils.spec.ts", testFilter, cwd)).toBe(true);
		});

		it("matches files in test directory", () => {
			expect(matchesFileFilter("/project/root/test/unit/utils.ts", testFilter, cwd)).toBe(true);
		});

		it("does not match regular source files", () => {
			expect(matchesFileFilter("/project/root/src/utils.ts", testFilter, cwd)).toBe(false);
		});
	});
	...
	describe("Elixir patterns", () => {
		...
		it("matches .ex files", () => {
			expect(matchesFileFilter("/project/root/lib/app.ex", elixirFilter, cwd)).toBe(true);
		});

		it("matches .exs files", () => {
			expect(matchesFileFilter("/project/root/test/app_test.exs", elixirFilter, cwd)).toBe(true);
		});

		it("does not match .eex files", () => {
			const filter = ["**/*.ex"];
			expect(matchesFileFilter("/project/root/lib/template.eex", filter, cwd)).toBe(false);
		});
	});
	...
	describe("Markdown patterns", () => {
		...
		it("matches .md files", () => {
			expect(matchesFileFilter("/project/root/docs/README.md", mdFilter, cwd)).toBe(true);
		});

		it("matches root README", () => {
			expect(matchesFileFilter("/project/root/README.md", mdFilter, cwd)).toBe(true);
		});

		it("does not match .mdx files", () => {
			expect(matchesFileFilter("/project/root/docs/page.mdx", mdFilter, cwd)).toBe(false);
		});
	});
	...
	describe("relative path handling", () => {
		...
		it("handles relative paths", () => {
			expect(
				matchesFileFilter("src/file.ts", ["src/**/*.ts"], cwd)
			).toBe(true);
		});
	});
});
...
describe("Tool Filter Matching", () => {
	...
	function matchesToolFilter(toolName: string | undefined, toolFilter: string[] | undefined): boolean {
...
describe("Async Hook Output Format", () => {
	...
	it("additionalContext instructs rerun without --async flag", () => {
		...
		expect(additionalContext).toContain("han hook run jutsu-typescript typecheck-async");
...
describe("Command Building", () => {
	...
	it("replaces ${HAN_FILES} placeholder", () => {
		...
		expect(buildCommandWithFiles(command, files)).toBe("biome check /path/to/file.ts");
	});

	it("handles multiple files", () => {
		const command = "eslint ${HAN_FILES}";
		const files = ["/path/to/a.ts", "/path/to/b.ts"];
		expect(buildCommandWithFiles(command, files)).toBe("eslint /path/to/a.ts /path/to/b.ts");
	});

	it("preserves command without placeholder", () => {
		const command = "mix format --check-formatted";
		const files = ["/path/to/file.ex"];
		expect(buildCommandWithFiles(command, files)).toBe("mix format --check-formatted");
	});
});
