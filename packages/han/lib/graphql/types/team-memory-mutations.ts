...
export const ShareLearningInput = builder.inputType('ShareLearningInput', {
  fields: (t) => ({
    content: t.string({
      required: true,
      description: 'Learning content to share',
    }),
    domain: t.string({
      required: true,
      description: 'Domain/category for the learning',
    }),
    source: t.string({
      required: false,
...
export const ExportOptionsInput = builder.inputType('ExportOptionsInput', {
  fields: (t) => ({
    approvedOnly: t.boolean({
      required: false,
      description: 'Only export approved learnings',
    }),
    domains: t.stringList({
      required: false,
      description: 'Filter by domains',
    }),
    includePatterns: t.boolean({
      required: false,
      description: 'Include aggregated patterns',
    }),
...
export const ImportOptionsInput = builder.inputType('ImportOptionsInput', {
  fields: (t) => ({
    duplicateStrategy: t.string({
      required: false,
      description: 'How to handle duplicates: skip, replace, create_new',
    }),
    autoApprove: t.boolean({
      required: false,
      description: 'Auto-approve imported learnings',
    }),
    domains: t.stringList({
      required: false,
    ...
    dryRun: t.boolean({
      required: false,
      description: 'Dry run without importing',
    }),
  }),
});
 ...
 */
const ShareLearningResultRef = builder.objectRef<ShareLearningResult>(
  'ShareLearningResult'
);
...
const ExportResultRef = builder.objectRef<ExportResultData>(
  'TeamKnowledgeExportResult'
);
...
const ImportResultRef = builder.objectRef<ImportResultData>(
  'TeamKnowledgeImportResult'
);
 ...
 */
const ModerationResultRef =
  builder.objectRef<ModerationResult>('ModerationResult');
 ...
 * Sharing policy type
 */
const SharingPolicyRef =
  builder.objectRef<OrgSharingPolicy>('OrgSharingPolicy');
...
const PolicyUpdateResultRef =
  builder.objectRef<PolicyUpdateResultData>('PolicyUpdateResult');
...
builder.mutationField('importTeamKnowledge', (t) =>
  t.field({
    ...
    args: {
      context: t.arg({ type: TeamUserContextInput, required: true }),
      importJson: t.arg.string({
        required: true,
        description: 'JSON string of export data',
      }),
    ...
    resolve: async (_parent, args) => {
      ...
      const result = await importTeamKnowledge(ctx, importData, {
        duplicateStrategy:
          (args.options?.duplicateStrategy as
            | 'skip'
            | 'replace'
