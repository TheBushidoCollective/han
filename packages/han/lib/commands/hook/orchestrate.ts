...
/**
 * Extract file path from PostToolUse payload based on tool type.
 * Supports Edit, Write, NotebookEdit tools.
 */
function extractFilePathFromPayload(payload: HookPayload): string | null {
	const toolName = payload.tool_name;
	if (!toolName) return null;

	// Get tool input from payload (if available)
	// The tool_input is the input parameters passed to the tool
...
/**
 * Check if a file matches the file_filter glob patterns.
 * Returns true if no filter is defined or if file matches any pattern.
 */
 ...
 * Returns true if no test is defined or if test exits 0.
 */
function passesFileTest(
	...
	try {
		execSync(fileTest, {
			...
			timeout: 5000, // 5 second timeout for content inspection
		});
		return true; // Exit 0 = pass
	} catch {
		return false; // Non-zero exit = fail
	}
}
...
async function handlePostToolUseAsync(
	...
	projectRoot: string,
	sessionId: string,
): Promise<{ queued: boolean; hookCount: number }> {
	// Extract file path from tool payload
	const filePath = extractFilePathFromPayload(payload);
	if (!filePath) {
		// No file path - nothing to queue
		return { queued: false, hookCount: 0 };
	}
	...
	for (const task of tasks) {
		...
		if (!task.hookDef.fileFilter || task.hookDef.fileFilter.length === 0) {
			continue;
		}

		for (const directory of task.directories) {
			// Check if file matches the filter patterns
			if (!matchesFileFilter(filePath, task.hookDef.fileFilter, directory)) {
				continue;
			}

			// Check if file passes the content test
			if (!passesFileTest(filePath, task.hookDef.fileTest, directory)) {
				continue;
			}
			...
			try {
				...
				console.error(
					`${colors.red}[PostToolUse]${colors.reset} Failed to queue ${task.plugin}/${task.hookName}: ${error}`,
				);
			}
		}
	}
...
export async function orchestrate(
	...
	// This enables immediate feedback on file changes without blocking the agent
	if (eventType === "PostToolUse" && !options.wait && !options.check) {
		// Filter tasks to those with fileFilter (async-capable)
		const asyncTasks = tasks.filter(
			(t) => t.hookDef.fileFilter && t.hookDef.fileFilter.length > 0,
		);

		if (asyncTasks.length > 0) {
			const { queued, hookCount } = await handlePostToolUseAsync(
				asyncTasks,
				payload,
				projectRoot,
				sessionId,
			...
			if (queued && isDebugMode()) {
				console.error(
					`${colors.dim}[PostToolUse]${colors.reset} Queued ${hookCount} async hook(s)`,
				);
			}

			// PostToolUse with fileFilter is purely async - no blocking output
			// The agent will see results at the next checkpoint (Stop or git commit)
			return;
		}

		// If no async tasks matched, fall through to normal (synchronous) handling
		// This covers hooks without fileFilter that should run immediately
	}
