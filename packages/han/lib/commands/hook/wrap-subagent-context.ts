...
function executeContextCommand(command: string): string {
	try {
		...
		if (isDebugMode()) {
			console.error(`[wrap-subagent-context] Error executing context command: ${error}`);
		}
		return "";
	}
}
...
function readContextFile(filePath: string): string {
	try {
		...
		if (isDebugMode()) {
			console.error(`[wrap-subagent-context] Error reading context file: ${error}`);
		}
	}
	return "";
}
...
async function wrapSubagentContext(options: {
	...
	if (!originalValue && toolName === "Task") {
		if (isDebugMode()) {
			console.error(`[wrap-subagent-context] No ${targetField} in tool_input, exiting`);
		}
		process.exit(0);
	}
	...
	if (originalValue.includes(`<${tag}>\n`)) {
		if (isDebugMode()) {
			console.error("[wrap-subagent-context] Already has context injected, exiting");
		}
		process.exit(0);
	}
	...
	if (!contextOutput) {
		if (isDebugMode()) {
			console.error("[wrap-subagent-context] No context gathered, exiting without modification");
		}
		process.exit(0);
	}
...
export function registerWrapSubagentContext(hookCommand: Command): void {
	hookCommand
		...
		.option("--tag <name>", "XML tag name to wrap context (default: subagent-context)")
