...
import { gitWorktreeList, type GitWorktree } from "../../../../han-native";
...
function parseYamlFromFile(
	...
	try {
		...
		const result = execSync(`han parse yaml ${key} -r --default "${defaultValue}"`, {
			input: content,
			stdio: ["pipe", "pipe", "pipe"],
		});
...
function parseAiDlcWorktreePath(worktreePath: string): {
	...
	const unitMatch = name.match(/^ai-dlc-([^-]+-[^-]+(?:-[^0-9][^-]*)*)-(\d+-.+)$/);
	if (unitMatch) {
		return {
...
export function discoverAiDlcWorktrees(repoRoot: string): AiDlcWorktree[] {
	...
	for (const wt of worktrees) {
		...
		let intentSlug = pathInfo.intentSlug || branchInfo.intentSlug;
		let unitSlug = pathInfo.unitSlug || branchInfo.unitSlug;
 ...
 */
export function discoverForConstruct(repoRoot: string): WorktreeDiscoveryResult {
	const worktrees = discoverAiDlcWorktrees(repoRoot);
 ...
 */
export function discoverAiDlcWorktreesUnified(repoRoot: string): AiDlcWorktree[] {
	const vcs = detectVcs(repoRoot);
	...
	for (const wt of worktrees) {
		...
		let branchInfo: { intentSlug?: string; unitSlug?: string; isAiDlc: boolean };
		if (vcs === "jj") {
