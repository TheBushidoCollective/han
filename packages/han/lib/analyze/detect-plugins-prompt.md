...
Plugins are organized into categories (the category is in the source path, but plugin names are simple):

- **Languages** - Language-specific skills (e.g., `typescript`, `rust`, `python`)
- **Frameworks** - Framework skills (e.g., `react`, `nextjs`, `django`)
- **Tools** - Build tools and utilities (e.g., `bun`, `playwright`, `jest`)
- **Validation** - Linters and validators (e.g., `biome`, `eslint`, `prettier`)
- **Patterns** - Development patterns (e.g., `tdd`, `atomic-design`, `bdd`)
- **Disciplines** - Development practices (e.g., `frontend`, `backend`, `api`)
- **Services** - External integrations (e.g., `github`, `linear`, `jira`)
- **Specialized** - Domain-specific tools (e.g., `ios`, `tensorflow`, `android`)
- **bushido** - Core quality principles (ALWAYS recommend this)
- **core** - Essential infrastructure (ALWAYS recommend this)
...
- If provided, you should:
  - **Understand why each plugin was likely added**: Look at plugin descriptions and infer original use case
    ...
    - Example: `frontend` suggests frontend focus
...
- Cross-reference detected technologies with available plugins
- ONLY recommend plugins from the list provided
- Aim for 3-8 total plugins that best match the codebase
- Always include "bushido" and "core" as they are essential plugins
...
```json
["bushido", "core", "typescript", "react", "frontend"]
```
