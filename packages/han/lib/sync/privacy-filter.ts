...
import { getSyncConfig, type SyncConfig as HanSyncConfig } from "../config/han-settings.ts";
import { DEFAULT_SYNC_CONFIG, type SyncConfig, type SyncEligibility } from "./types.ts";
...
export function matchesPattern(remote: string, patterns: string[]): boolean {
	...
	const normalizedRemote = `${parsed.host}/${parsed.owner}/${parsed.repo}`.toLowerCase();
...
export function checkBatchEligibility(
	...
	for (const { session, repo } of sessions) {
		results.set(session.id, checkSyncEligibility(session, repo, syncConfig, userId));
	}
 ...
 */
export function getEligibilitySummary(eligibility: Map<string, SyncEligibility>): {
	eligible: number;
	excluded: number;
	personal: number;
	organization: number;
