...
export function getRecentSessions(
  limit = LIMITS.DEFAULT_RECENT_SESSIONS
): RecentSession[] {
    ...
    for (const session of recentSessions) {
      try {
        ...
        if (process.env.HAN_DEBUG) {
          console.warn(
            `[recent_sessions] Failed to read session ${session.sessionId}:`,
  ...
  try {
    ...
    for (const { slug, filePath, mtime } of sortedTranscripts) {
      ...
      try {
        ...
        await new Promise<void>((resolve, _reject) => {
          ...
          rl.on('line', (line) => {
            ...
            try {
              ...
              if (process.env.HAN_DEBUG) {
                console.warn(
                  `[grep] Skipping unparseable line ${lineNum} in ${filePath}`
                );
              }
            }
          });
        ...
        if (process.env.HAN_DEBUG) {
          console.warn(
            `[grep] Failed to process ${filePath}:`,
