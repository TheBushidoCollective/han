...
): { isAdmin: boolean; role?: AdminRole } {
...
export function getModerationStats(context: UserPermissionContext): {
	...
	approved: number;
	rejected: number;
	byDomain: Record<string, { pending: number; approved: number; rejected: number }>;
} | null {
	if (!context.orgId) {
		return null;
	}
	...
	for (const learning of learnings) {
		...
		if (!stats.byDomain[learning.domain]) {
			stats.byDomain[learning.domain] = { pending: 0, approved: 0, rejected: 0 };
		}
