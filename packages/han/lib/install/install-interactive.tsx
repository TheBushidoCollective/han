...
// Placeholders for HTML entities during markdown parsing
...
export function parseMarkdown(text: string): string {
	try {
		// Replace HTML entities with placeholders to preserve them through marked parsing
		// This prevents &lt;div&gt; from becoming <div> and then being stripped as an HTML tag
		const processed = text
			.replace(/&lt;/g, ENTITY_PLACEHOLDERS.lt)
			.replace(/&gt;/g, ENTITY_PLACEHOLDERS.gt)
			.replace(/&quot;/g, ENTITY_PLACEHOLDERS.quot)
			.replace(/&#39;/g, ENTITY_PLACEHOLDERS.apos)
			.replace(/&amp;/g, ENTITY_PLACEHOLDERS.amp);
		...
		if (typeof parsed === "string") {
			...
			return (
				parsed
					// Strip HTML tags generated by marked (e.g., <p>, <strong>, <a>)
					.replace(/<[^>]*>/g, "")
					// Strip link URLs in parentheses (e.g., "text (https://...)" -> "text")
					.replace(/\s*\(https?:\/\/[^)]+\)/g, "")
					// Restore placeholders to actual characters
					.replace(new RegExp(ENTITY_PLACEHOLDERS.lt, "g"), "<")
					.replace(new RegExp(ENTITY_PLACEHOLDERS.gt, "g"), ">")
					.replace(new RegExp(ENTITY_PLACEHOLDERS.quot, "g"), '"')
					.replace(new RegExp(ENTITY_PLACEHOLDERS.apos, "g"), "'")
					.replace(new RegExp(ENTITY_PLACEHOLDERS.amp, "g"), "&")
					// Also decode any HTML entities that marked might have generated
					.replace(/&lt;/g, "<")
					.replace(/&gt;/g, ">")
					.replace(/&quot;/g, '"')
					.replace(/&#39;/g, "'")
