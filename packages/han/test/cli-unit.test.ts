...
describe("CLI Unit Tests", () => {
	...
	describe("Metrics subcommand help", () => {
		test("shows session-start command", () => {
			const output = captureOutput();
			const program = makeProgram({
				exitOverride: true,
				writeOut: output.writeOut,
			...
			try {
				program.parse(["node", "han", "metrics", "--help"]);
			} catch {
				// expected
			}
		...
		test("shows session-end command", () => {
			const output = captureOutput();
			const program = makeProgram({
				exitOverride: true,
				writeOut: output.writeOut,
			...
			expect(stdout).toContain("session-end");
		});

		test("shows detect-patterns command", () => {
			const output = captureOutput();
			const program = makeProgram({
				exitOverride: true,
				writeOut: output.writeOut,
			...
			expect(stdout).toContain("detect-patterns");
		});

		test("shows session-context command", () => {
			const output = captureOutput();
			const program = makeProgram({
				exitOverride: true,
				writeOut: output.writeOut,
